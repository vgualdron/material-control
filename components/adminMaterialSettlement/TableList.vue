<template>
  <b-card
    title="Administrar Liquidaciones de Material"
    sub-title="Opciones de listar, editar liquidaciones de material">
...
export default {
  ...
  data () {
    return {
      ...
      fields: [
        ...
        { key: 'royalties', label: 'Regalias', sortable: true, class: 'text-center' },
  ...
  computed: {
    ...
    items () {
      return this.adminMaterialSettlements.data.map((item) => {
        return {
          ...item,
  ...
  methods: {
    ...mapActions(types.PATH, {
      getAdminMaterialSettlement: types.actions.GET_ADMIN_MATERIAL_SETTLEMENTS,
      setAdminMaterialSettlement: types.actions.SET_ADMIN_MATERIAL_SETTLEMENT,
      setShowModalForm: types.actions.SET_SHOW_MODAL_FORM,
      setTypeAction: types.actions.SET_TYPE_ACTION
    }),
    showModal (item, action) {
      if (action !== 'create') {
        this.setAdminMaterialSettlement({ ...item });
      }
    ...
    search () {
      ...
      this.getAdminMaterialSettlement(data);
    }
  }
};
</script>
